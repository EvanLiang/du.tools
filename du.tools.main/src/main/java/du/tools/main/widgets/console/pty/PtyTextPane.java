...
public class PtyTextPane extends JETextPane implements Terminal {

    private Font defFont = new Font("monospaced", 0, 14);
    ...
    private void setPtySize() {
        int charWidth = getFontMetrics(defFont).charWidth(' ');
        int columns = getVisibleRect().width / charWidth;
        pty.resize(columns - 2, 100);
    }
    ...
    private void startOutputThread() {
        Runnable runnable = new Runnable() {
            public void run() {
                XtermProcessor runner = new XtermProcessor(pty, PtyTextPane.this);
                try {
                    while (!exit) {
                        runner.process();
                    }
    ...
    public void deleteLine() {
        try {
            ...
            int index = lineOffsetStart(lastLine);
    ...
    public static void main(String args[]) throws IOException, InterruptedException {
        ...
        final PtyTextPane c = new PtyTextPane();
